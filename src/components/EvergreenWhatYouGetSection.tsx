          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
            {blocks.map((block, index) => {
              ...
              return (
                <div 
                  key={index} 
                  className="bg-white rounded-xl p-4 sm:p-5 lg:p-6 border border-border shadow-soft hover:shadow-warm transition-all duration-300 hover:-translate-y-1"
                >
                  <div className="flex items-center space-x-3 mb-3 sm:mb-4">
                    <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-saffron to-deep-saffron rounded-lg flex items-center justify-center flex-shrink-0">
                  ...
                  <ul className="space-y-1.5 sm:space-y-2">
                    {block.items.map((item, itemIndex) => (
                      <li key={itemIndex} className="flex items-start space-x-2">
                        <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-green-accent rounded-full mt-1.5 sm:mt-2 flex-shrink-0"></span>
